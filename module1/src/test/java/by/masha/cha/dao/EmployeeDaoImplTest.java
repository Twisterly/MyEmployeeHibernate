...
public class EmployeeDaoImplTest extends BaseDaoTest {
    ...
    public void create() {
        ...
        IDataSet departmentDataSet = new FlatXmlDataSetBuilder()
                .build(EmployeeDaoImplTest.class.getResourceAsStream(
                        "DepartmentDaoImplTest.xml"));
        DatabaseOperation.CLEAN_INSERT.execute(iDatabaseConnection, departmentDataSet);

        ResultSet rs = conn.createStatement().executeQuery("select count(*) from t_employee;");
        rs.next();
        ...
        //Then
        rs = conn.createStatement().executeQuery("select count(*) from t_employee;");
        rs.next();
    ...
    public void findById() {
        ...
        //When
        Employee employee = targetObject.findById(101L);
